...
Current pipeline: read header directives (default `bolha`) -> extract metadata takes -> resolve conflicts -> parse the remaining module with the TypeScript parser -> collect AST facts and comments -> execute declared requirement rules -> emit JavaScript -> build the social artifact.
...
The compiler uses `transpileModule` for isolated files. It does not provide semantic checking across files. Source maps remapped to `.bolhes` and the Ruby/Python parser are pending.
