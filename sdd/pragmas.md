...
O arquivo canonico e `packages/pragmas/registry.yml`. O build valida o schema e gera `registry.json`, consumido pela API publica de `@bolhes/pragmas`. O catalogo contem os 30 IDs do inventario, aliases confirmados e handles incertos preservados como `TBD` ou `null`.

O validator rejeita chaves desconhecidas, campos ausentes, tipos incorretos, IDs e aliases duplicados, handles invalidos, recusas sem destino e conflitos duplicados ou autorreferentes. Erros apontam o caminho do campo.
